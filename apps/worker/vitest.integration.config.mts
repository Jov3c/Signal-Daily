...
/**
 * Worker 集成测试专用配置 —— 需要**真实 MySQL + 真实 Redis**。
 *
 * 运行：
 *   pnpm --filter @signal/worker test:integration
 *
 * 与默认 `pnpm test` 分开的原因（与 Agent 01 的 `pnpm test:db`、
 * Agent 03 的 `vitest.integration.config.mts` 同一个理由）：
