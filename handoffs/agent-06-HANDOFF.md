...
### 设计取舍（§23.8 要求逐条记录 —— 审查时这些只存在于代码注释里）

> 编号说明：本节写于 2026-09-24 §23 简化之前，当时「真 bug vs 设计取舍」是 §23.6，
> 简化后为 §23.8。
...
按当时的 §23.2 做的是**深度审查**（两轮：安全向 + 工程向，由未参与本次开发的独立执行者完成），
这是 §23 简化前的做法 —— 新版默认只做一次轻量审查（§23.4），深度审查改为按需。
本次共消耗约 52 万 token、约 2 小时，这也是它被简化的原因。
